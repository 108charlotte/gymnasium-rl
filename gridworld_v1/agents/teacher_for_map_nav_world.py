...
class DQN(torch.nn.Module): # CNN because should have spatial reasoning
    def __init__(self, num_types_special_regions, num_directions, num_filters_first_layer, final_conv_filters, target_spatial_size):
        ...
        self.dx_dy_processing = nn.Sequential(
            nn.Linear(2, 4), 
            # actually I don't want activation at all; nn.Tanh() # don't want ReLU bc want to preserve if to left/right
        )

        self.to_action = nn.Sequential(
            nn.Linear(final_conv_filters * target_spatial_size**2 + 4, 64), # spatial out + linear out
            nn.ReLU(), 
            nn.Linear(64, 4)
        )
    ...
    def forward(self, x):
        ...
        dx_dy_result = self.dx_dy_processing(x[:, self.num_spatial_channels:, 0, 0])
        return self.to_action(torch.cat([spatial_result, dx_dy_result], dim=1))
