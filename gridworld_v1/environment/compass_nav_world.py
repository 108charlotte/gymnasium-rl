...
class GridWorldBase(gym.Env): 
    ...
    def get_student_teacher_alignment_reward(self, student_action, teacher_action): # to make this work, I'm going to need to store the teacher's last action since step_one_agent only moves one...maybe I'll make a step_both_agents for this
        ...
        scaled_dot = dot * self.student_teacher_alignment_multiplier
        return scaled_dot
...
class StudentWrapper(gym.Wrapper): 
    def __init__(self, env, visibility : int, max_steps: int = 50, special_region_rewards: list[float] = []): # if visibility not passed just sees whole world, defaults to ignoring regions
        ...
        self.env = env
        self.visibility = visibility
        num_channels = 1+env._num_types_special_regions+1+self.env.num_directions+1 # agent + one per region type + target + angle (one-hot encoded) + teacher
        grid_size = 2*visibility + 1
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(num_channels, grid_size, grid_size), dtype=np.float32)
    ...
    def step(self, action): 
        full_obs, rewards, terminations, _, info = self.env.step_both_agents(action, action) # student and teacher don't do the same thing, this is a placeholder 
        num_steps += 1
